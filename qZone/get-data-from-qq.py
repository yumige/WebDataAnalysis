...
class qqlogin:
    ...
    def GetVerifyCode(self):
        ...
        par = {
            ...
            'js_ver':10034,
            'js_type':1,
            'login_sig':'OYHnhJ-yrkljcXllHNv1eIfoRtd7YpJHqv1XLbhwOfyEuFPmQd1eKWOeUamNkoIG',
            'u1': self.izone,
            'r': 0.5670551517978311
        }
        url = "http://check.ptlogin2.qq.com/check?%s" % urllib.parse.urlencode(par)
        print("请求是否需要输入验证码：" + url)
        check = Http(url)
        #verify =  eval(check.split("(")[1].split(")")[0])
        #verify = list(verify)
        #print(verify[2].encode('utf8'))
        #res = self.fetch(url).text
        verify = re.findall('\'(.*?)\'', check)
        self.ucode = verify[2] 
        if verify[0] == "1":
    ...
    def Login(self,uid,password,verifycode):
        qq = qqlib.QQ(uid, password);
        par = {
            'u': self.uin,
            'verifycode':verifycode,
            'pt_vcode_v1':0,
            'pt_verifysession_v1': GetCookie('ptvfsession'),
            'p': qq.pwdencode(verifycode, self.ucode, password),
            'u1': self.izone,
            'ptredirect':0,
            'h':1,
            't': 1,
    ...
    def __init__(self,uin,password):
        self.izone = 'http://qzs.qq.com/qzone/v5/loginsucc.html?para=izone'
        self.uin = uin  #账号、密码赋值
        self.ucode = ''
        self.password = password
        self.session = requests.Session()
        self.captcha = self.GetVerifyCode() #获取验证码
        print('验证码是:' + self.captcha)
        self.Login(self.uin,self.password,self.captcha) #登录
        qq = qqlib.QQ(uin, password);
        login_resault = qq.login()
        fetch_resault = qq.fetch('http://user.qzone.qq.com/732952649/')
        string_result = fetch_resault.text.encode('utf8')
        resault_file = open('fetch_resault.html','w')
        #print(string_result.decode())
        resault_file.write(string_result.decode())

#程序入口
if __name__ == "__main__":
    print("By：Sangdi和她的小伙伴们")
    uin = input("请输入QQ号码:\n").strip()   #输入QQ账号
    print("请输入QQ密码:")                   #输入密码
    password = pwd_input().strip()
    #uin = ''
